...
void* connection_handler(void* args) {
    ...
    for (;;) {
        ...
        case ASYNC_PROTOCOL_ADD_PARAGRAPH_ID:
            update_paragraph_protocol(
                sock, paragraphs, client_message, ASYNC_PROTOCOL_ADD_PARAGRAPH_ID);
            break;
        ...
        case UNLOCK_PARAGRAPH_PROTOCOL_ID:
            update_paragraph_protocol(
                sock, paragraphs, client_message, UNLOCK_PARAGRAPH_PROTOCOL_ID);
            break;
...
void update_paragraph_protocol(
    ...
    case ASYNC_PROTOCOL_ADD_PARAGRAPH_ID:
        unlock_paragraph(paragraphs, paragraph_number, sock);
        linked_list_insert_after_node_number(
            paragraphs, paragraph_number, linked_list_create_node("\n"));
...
void async_protocol_delete_paragraph(
    ...
    int paragraph_number = extract_paragraph_number(client_message);
    char* content_of_paragraph_to_delete =
        get_content_of_paragraph(paragraphs, paragraph_number);
    char* content_of_paragraph_above =
        get_content_of_paragraph(paragraphs, paragraph_number - 1);
    content_of_paragraph_above[strcspn(content_of_paragraph_above, "\n")] =
        '\0';  // trim '\n'

    size_t new_size =
        strlen(content_of_paragraph_above) + strlen(content_of_paragraph_to_delete) + 1;
    ...
    edit_content_of_paragraph(
        paragraphs, paragraph_number - 1, content_of_paragraph_above);
...
char* create_message_with_lock_status(LinkedList* paragraphs) {
    ...
    char* message = calloc(2, sizeof(char));

    while (temp != NULL) {
        if (temp->locked == 1) {
            char number[16];
            sprintf(number, "%d,", current_number);
            char* reallocated_message =
                realloc(message, strlen(message) + strlen(number) + 1);
